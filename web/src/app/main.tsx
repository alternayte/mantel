 ...
 */
type Screen = { name: 'albums' } | { name: 'album'; id: string } | { name: 'settings' }
...
function Creator() {
  ...
  if (screen.name === 'album') return <Album id={screen.id} onBack={() => go({ name: 'albums' })} />
  ...
  return <Albums onOpen={(id) => go({ name: 'album', id })} onSettings={() => go({ name: 'settings' })} />
}
